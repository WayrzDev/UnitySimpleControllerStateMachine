    ...
    public void Move(Vector2 input, float speedMultiplier = 1f)
{
    ...
    public void Jump()
    {
        if (CanJump())
        {
            Velocity = new Vector3(Velocity.x, Mathf.Sqrt(JumpForce * -2f * Gravity), Velocity.z);
    ...
    private void ApplyGravity()
{
    ...
    {
        Vector3 groundCheckPos = transform.position + Vector3.down * 
            (CharacterController.height / 2f - CharacterController.skinWidth + 0.05f);
...
{
    if (_isOnSlope && !Input.GetButton("Jump"))
    {
